...
(() => {
  ...
  function parseRoute() {
    ...
    const [name, ...rest] = raw.split('/').map((part) => decodeURIComponent(part));
  ...
  function atlasNodes(focus) {
    ...
    const paperNodes = positions.map(({ paper, x, y }) => {
      ...
      return `<g class="atlas-node paper" tabindex="0" role="link" aria-label="Open ${escapeHTML(paper.title)}" data-nav="paper/${escapeHTML(paper.id)}">
    ...
    const focusNode = `<g class="atlas-node focus" tabindex="0" role="link" aria-label="Open ${escapeHTML(focus.label)}" data-nav="focus/${escapeHTML(focus.id)}">
  ...
  function renderAtlas(selectedId) {
    ...
    const body = `
      ...
      <section class="atlas-panel">
        ...
        <svg class="atlas-canvas" viewBox="0 0 900 520" role="img" aria-label="Paper membership map for ${escapeHTML(selected.label)}">${atlasNodes(selected)}</svg>
  ...
  function buildBriefText() {
    ...
    const paperLines = selected.length
      ? selected.map((paper, index) => `${index + 1}. ${paper.title} (${paper.year})\n   ${paper.url}\n   Relevance: ${paper.relevance.join(', ')}`).join('\n')
...
Generated by a human from Sabine Research Desk 95 using the public snapshot dated ${D.research.snapshot_date}. Verify time-sensitive claims against live primary sources.`;
  ...
  document.addEventListener('keydown', (event) => {
    if ((event.key === 'Enter' || event.key === ' ') && event.target.matches('.atlas-node[data-nav]')) {
      event.preventDefault();
